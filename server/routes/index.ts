...
export async function registerRoutes(app: Express) {
  ...
  app.post("/api/bookings", async (req, res) => {
    try {
      const { barberId, customerName, start, end, serviceId } = req.body;

      // Check for conflicts
      const existingBookings = await storage.getBookingsByBarber(barberId);
      ...
      const hasConflict = existingBookings.some(booking => {
        ...
        return (
          (newStart >= bookingStart && newStart < bookingStart) ||
      ...
      try {
        const token = await storage.getGoogleToken('admin'); // Assuming admin user for barber
        if (token) {
          ...
          const event = {
            ...
            description: `Barbershop booking for ${customerName}`,
            start: {
          ...
          if (calendarResponse.data.id) {
            ...
            console.log(`✅ Created Google Calendar event: ${calendarResponse.data.id}`);
          }
        } else {
          console.log('⚠️  No Google token found - booking created without calendar sync');
        }
      } catch (calendarError) {
